...
export function CameraRig({
  ...
  useFrame(({ camera, pointer }, dt) => {
    ...
    const azimuth =
      angle === "side"
        ...
        : angle === "three-quarter"
          ? Math.PI / 4
          : 0;
    const parallax = reducedMotion || paused ? 0 : 0.17;
    camera.position.x = damp(
      camera.position.x,
      Math.sin(azimuth) * distance + pointer.x * parallax,
      2,
      dt,
    );
    ...
    camera.position.z = damp(
      camera.position.z,
      Math.cos(azimuth) * distance + intro,
      2,
      dt,
    );
    const tx = garden || macro || mobile ? 0 : -0.75;
