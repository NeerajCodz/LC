...
[Blue Passionflower](docs/specimens/passionflower-research.md) adds separately colored sepal shells, a four-series filament corona, raised anthers and styles, gland-bearing petioles and continuous palmate leaves. The internal `StemComponent` slot supports its climbing shoot and coiled tendrils. `supportHeight` moves the fixed bending boundary to the vine attachment; the free peduncle and head share that deformation. Other species retain their rooted cantilever behavior. Specialized organs can consume the existing pulse and cursor refs without React updates inside animation frames.

Mobile keeps a stable DPR request of 1 through macro transitions, with lower ratios when the shared pixel budget requires them. Macro increases mesh detail without reallocating a larger framebuffer. This avoids the blank composited macro view reproduced in mobile WebKit at fractional upscaling and keeps mobile fill cost at its normal-view level; MSAA remains enabled.
